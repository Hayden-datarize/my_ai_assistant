import { parseEvaluationResponse, parseQuestionResponse } from '../utils/gemini-parse';
...
export async function evaluateAnswer({ apiKey, question, answer }: EvaluateAnswerInput): Promise<{ score: number; feedback: string }> {
  ...
  const text = await generateText({ apiKey, prompt });
  // v3.41 T2 (Codex P1 F2): parseJsonText 직접 호출 → parseEvaluationResponse
  // (try/catch + score clamp 1-5 + feedback string guard, fallback {3, ''}).
