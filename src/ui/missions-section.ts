...
function renderCard(m: MissionInstance): string {
  ...
  const pbHtml = def.target >= 2
    ? `<span class="mission-card__progress" role="progressbar" aria-valuenow="${m.progress}" aria-valuemax="${def.target}" aria-label="${m.progress}/${def.target}"><span class="mission-card__bar" style="width:${Math.min(100, (m.progress / def.target) * 100)}%"></span><span class="mission-card__count">${m.progress}/${def.target}</span></span>`
    : '';
  const checkHtml = m.completed ? '<span class="mission-card__check" aria-label="완수">✓</span>' : '';
  // v3.24 T3: indent 압축 (production-safe).
  return `<li class="${cls}" aria-label="${escapeHtml(ariaLabel)}"><span class="mission-card__title">${escapeHtml(def.text)}</span>${pbHtml}<span class="mission-card__reward">+${def.rewardXp} XP</span>${checkHtml}</li>`;
}
