...
test.describe('v3.29 T3 dynamic tab load', () => {
  // v3.30 T5 (v3.29 carry C3): N=1 → N=3 격상. cold-start chunk 로드 안정성 회귀.
  // Playwright 1.59의 test.describe.configure는 mode/retries/timeout만 지원 (repeatEach X).
  // repeatEach는 testConfig project-level 옵션 — describe-level은 for-loop fallback 사용.
  for (let i = 0; i < 3; i++) {
    test(`cold-start home → archive → stats — 각 navigate 후 chunk 로드 + render (run #${i + 1})`, async ({ page }) => {
