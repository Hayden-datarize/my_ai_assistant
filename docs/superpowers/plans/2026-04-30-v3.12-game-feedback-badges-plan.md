...
### §0.1 CSS Append Convention (T7 review fix `1dd4362` 이후 적용)

T7 review에서 layer 컨벤션 위반 발견 (v3.3.0 cascade-layers refactor가 모든 rule을 `@layer` 안에 두기로 결정한 것을 T7가 위반). Fix commit `1dd4362`이 T7 추가 block을 `@layer components { ... }` wrapper로 감쌌음. **T8~T14에서 "Append to `src/styles/main.css`"로 명시된 모든 reward CSS는 이 wrapper 내부 끝에 append해야 함**. 새로 layer 외부에 추가 금지.

또한 이 review에서 발견된 token alignment:
- 색상은 `var(--success)` 사용 (codebase에 `--color-success`는 없음. fallback 불필요 — 토큰 unconditional 정의)
- z-index는 `var(--z-toast)` (모달과 함께 layer되어야 할 reward 모션) 또는 `var(--z-modal)` (modal 본체)
- 다른 design token은 `tokens.css` 먼저 grep으로 존재 검증 후 사용
...
- [ ] **Step 5-bis: wiring-gap `DEFERRED_LISTEN` allowlist 제거 (T3 review reminder)**

T3에서 dispatcher 4개 도입 + listener 0개라 `tests/lint/wiring-gap.spec.ts:48-58`에 `DEFERRED_LISTEN` allowlist를 임시 추가했음 (`V32_DEFERRED_EVENTS` 패턴). 본 task에서 `rewards.ts`가 4 listener 모두 도입하므로 allowlist 제거 + strict pairing 복원.

`tests/lint/wiring-gap.spec.ts`에서 `DEFERRED_LISTEN` 정의 + 사용 모두 제거. 4 reward 이벤트가 dispatcher↔listener 1:1 매칭으로 자연 통과해야 함.

```bash
npm test -- tests/lint/wiring-gap.spec.ts
```

Expected: PASS — `DEFERRED_LISTEN` allowlist 없이도 0 gap.

- [ ] **Step 6: Commit**

```bash
git add src/ui/rewards.ts src/main.ts tests/unit/ui/rewards.spec.ts tests/lint/wiring-gap.spec.ts
git commit -m "$(cat <<'EOF'
...
> **T6 review memo**: T6 implementer가 `removeToast` 250ms exit animation을 sync test 호환을 위해 제거했음. T8 motion 구현 시 `.toast-leaving` 클래스 + CSS `transition/animation` 재도입 가능 (test 영향 없도록 dismiss path 분리). 본 task에서 가능하면 함께 복원, 불가능하면 v3.13 polish backlog.
...
**Files:**
- Modify: `src/ui/modals/badge-detail.ts` (T6에서 8-line stub 미리 생성됨 — Write로 overwrite)
...
- [ ] **Step 5-bis: onboarding.ts v2 shape sync (P2-1, T2 review에서 발견)**

`src/ui/onboarding.ts`에서 신규 user 생성 시 `level: 1` 등 v1 shape literal을 사용 중. lazy migrate가 다음 read에서 자동 처리하지만, 코드 일관성을 위해 본 task에서 함께 정리.

`src/ui/onboarding.ts`의 `localStorage.setItem(STORAGE_KEY_USER, JSON.stringify(...))` 부근에서 새 user object literal:

기존 (v1 shape):
```ts
const user = { name, interests, onboardedAt, streak: 0, lastActiveDate: '', xp: 0, level: 1 };
```
...
> 정확한 line은 `grep -n "level: 1\|onboardedAt" src/ui/onboarding.ts`로 확인.
...
```bash
git add src/ui/modals/welcome-gamification.ts src/ui/handlers/home.ts src/ui/onboarding.ts tests/unit/ui/welcome-gamification.spec.ts
git commit -m "$(cat <<'EOF'
...
- [ ] **Step 5-bis: smoke fixture v2 shape 일괄 sync (P2-2, T2 review에서 발견)**

기존 Playwright smoke fixture 7곳이 `level: 1` 등 v1 shape literal로 user를 seed하고 있음. lazy migrate가 functional 동작은 보장하지만 코드베이스 일관성 위해 본 task에서 일괄 정리.

대상:
- `tests/smoke/briefings-empty.spec.ts:27,66`
- `tests/smoke/settings-delete-all.spec.ts:24`
- `tests/smoke/tier-downgrade.spec.ts:44`
- `tests/smoke/cardnews.spec.ts:37`
- `tests/smoke/settings-translation.spec.ts:18,50`
- `tests/smoke/polish.spec.ts:9`
- `tests/smoke/heatmap.spec.ts:27`

각 위치의 `localStorage.setItem('user', JSON.stringify({ ..., level: 1, ... }))` 패턴을 다음으로 교체:

```ts
localStorage.setItem('user', JSON.stringify({
  // ... 기존 필드 (name/interests/onboardedAt/streak/lastActiveDate/xp) 유지 ...
  earnedBadges: {},
  gamificationMigrated: true,
  schemaVersion: 2,
}));
...
> 주의: 각 spec의 streak/xp/interests 등 기존 값은 그대로 유지 (테스트 의도 보존). `level: N` 줄만 삭제하고 v2 신규 3 필드만 추가.

```bash
grep -rn "level:" tests/smoke/ | grep -v "//"  # 변경 후 0건이어야 함 (v3.12 reward toast spec의 level-up은 별개)
npx playwright test --reporter=list
```

Expected: smoke 89 (87 prior + 본 task 신규 2) PASS.

- [ ] **Step 6: Commit**

```bash
git add tests/smoke/v3.12-game-feedback.spec.ts tests/smoke/briefings-empty.spec.ts tests/smoke/settings-delete-all.spec.ts tests/smoke/tier-downgrade.spec.ts tests/smoke/cardnews.spec.ts tests/smoke/settings-translation.spec.ts tests/smoke/polish.spec.ts tests/smoke/heatmap.spec.ts docs/superpowers/specs/2026-04-29-v3.12-game-feedback-badges-design.md docs/superpowers/plans/2026-04-30-v3.12-game-feedback-badges-plan.md
git commit -m "$(cat <<'EOF'
...
**검토 적용 이력 (2026-04-30 T2 review 후 추가)**:

- **T2 review P2-1**: `src/ui/onboarding.ts` 신규 user 생성 시 v1 shape (`level: 1`) 사용 → T13 step 5-bis로 흡수 (v2 shape + `gamificationMigrated: true` set, fresh user는 backfill 불필요).
- **T2 review P2-2**: Playwright smoke fixture 7곳 v1 shape seed → T15 step 5-bis로 흡수 (일괄 sync, 의미 보존).
- **T2 review P2-3** (`migrateUserToV2(raw: any)` → `unknown`): plan/spec deviation 회피 위해 v3.13 backlog. sibling `migrateAnswer(raw: unknown)` 컨벤션 정렬은 다음 사이클 polish 시 일괄.
- **T2 implementer concern 2** (lazy migrate setItem + 후속 saveUser 비-원자성): T4 wiring 시점에 review. functional 영향 = 일시적 xp delta 손실만 (재답변 시 회복), data corruption 없음. 본 사이클 P0/P1 아님 → backlog (v3.13 polish 시 단일 setItem coalesce 검토).

---

## §9 Retroactive Sync (T15에서 추가)

본 plan 실행 중 발생한 review fix + drift 항목들의 retroactive 기록. spec §9와 동기화.

| Review fix | Commit | 적용 위치 |
|---|---|---|
| T2 review P2-1 (onboarding v2 shape) | `1e6041c` | T13에 흡수 (`src/ui/onboarding.ts:188-200`) |
| T5 review P1-A (interest keyword 매칭) | `3310578` | `src/state/achievements.ts:9-28` (interestKeywords 헬퍼 + KO 5-type) |
| T7 review P1/P2 (z-index/token/layer) | `1dd4362` | `src/styles/main.css` `@layer components` wrapper |
| T11 review P1 (legacy CSS + a11y) | `4d4b721` | `src/styles/pages/stats.css` 정리 + `src/ui/handlers/stats.ts:288` aria |
| T13 review P1-1 (close+switch 순서) | `6d5d061` | `src/ui/modals/welcome-gamification.ts:79-84` |
| T15 v20-compat fix | (T15 commit) | `tests/smoke/v20-compat.spec.ts:26-32` (legacy v2.0 user에 `gamificationMigrated: true` 추가) |

### Token/CSS 컨벤션 정착 (T7 → T8~T14)

- 토큰: `var(--success)` / `var(--text-primary)` / `var(--text-secondary)` / `var(--bg-card)` / `var(--border)` / `var(--z-toast)` / `var(--z-modal)` 사용 (plan 명시 `--color-*` / `--text` / `--color-text-muted` 모두 미존재 토큰).
- CSS: 모든 reward styles는 `src/styles/main.css`의 단일 `@layer components { ... }` wrapper 안. T7 fix `1dd4362`이 도입한 패턴.

### Smoke fixture v2 sync (P2-2 → T15 흡수)

15 spec 파일 / 19 위치의 user shape `level: 1` → v2 (`earnedBadges: {}`, `gamificationMigrated: true`, `schemaVersion: 2`). sed 일괄 처리. v20-compat:9 (legacy v2.0 의도 보존)는 단일 `gamificationMigrated: true` flag만 추가해 T13 modal 회피.

### 카운트 (T15 측정)

- vitest unit: 506 PASS (single run baseline) · N=5 unit/ui 5/5 안정
- Playwright smoke: 신규 v3.12 2 PASS + sync 후 fixture 재테스트 통과
- bundle JS gzip: ≈ 27.5 kB (절대값 — v3.11 baseline 미기록)

### v3.13 backlog (본 사이클에서 의도적으로 미처리)

- T2 implementer concern 2: lazy migrate setItem + saveUser 비-원자성 (xp delta 일시 손실, data corruption 없음)
- T5 P2-3: `migrateUserToV2(raw: any)` → `unknown` (sibling 컨벤션 일치)
- T5 P2-F: `labor_law` keyword VS16 stray (functional 영향 없음, cosmetic)
- T5 false-positive 잔존: 영문 짧은 ID (`pm`/`data`/`startup` 등)이 RSS body substring 매칭 가능 (over-credit)
- T7 P2-3: timer dedup (production-likelihood 낮음)
- T7 P2-5: `__resetForTest` `clearAllTimers` (test 사이 timer race)
- T11 P2: hydrateBadges 함수 size, lazy import latency, BADGE_CATALOG filter 5×, escapeText 중복
- T13 P2: backfill saveUser race, goStatsBtn document scope
- vitest count DoD `+100 ±3` ↔ 실제 +111 (453 → 564) — v3.12.1 T5에서 retro sync 완료
